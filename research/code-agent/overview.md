...
A/B 源码专题已有输入权威化、模型请求重建、工具闭合、reactive compact、冷恢复、Memory 并发保护、大结果完整性和 artifact 生命周期的源码追踪。B5 的 Reactive 此前埋在表格与末尾说明中，不能算整体流程验收通过；现已补主图回环和独立步骤，仍待读者复核。基于源码可确认的是控制流与契约；Provider 外部行为、仓库外清理和具体远端部署下的路径可达性保持有边界未知。跨 Host owner/lease、子 Agent 与扩展机制尚未作为本批专题研究。
