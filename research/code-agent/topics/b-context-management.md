...
**Memory 是一个需要跨边界读的例子。**产品设置允许后，主模型看的是 Memory 使用规则与启动时的 `MEMORY.md` 索引；成功 turn 后可启动独立的后台提取。提取调度用持久 Session 快照判定“是否值得做”和计算消息数，实际 Memory Agent 请求却继承当前 Runtime 模型历史及完整工具目录，再追加提取指令；工具权限在执行时收窄。这意味着主 Runtime 已压缩的历史不会因持久快照在提取请求中自动恢复全文，且“仅处理最近 N 条”主要是提示词约束。Memory Agent 自己未进入普通 turn 的 micro/auto/reactive compact 环；新事实写盘后，热 Runtime 自动注入的索引快照也不会因此立即重读。旧报告把这几层折成一条“读取持久消息 → 提取 → 下次可见”的箭头，无法用于设计同类系统；更完整流程见 [B3](b-memory.md)。[两份快照](../../../apps/zcode-cli/packages/core/src/runtime/helpers/project-memory-extraction.ts) · [模型实际输入](../../../apps/zcode-cli/packages/core/src/runtime/helpers/project-memory-agent.ts) · [上下文索引加载](../../../apps/zcode-cli/packages/core/src/runtime/methods/context.ts)
